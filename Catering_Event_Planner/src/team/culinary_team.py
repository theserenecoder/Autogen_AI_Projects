...
from src.agents.user_proxy import getCulinaryTeamUserApproval
from utils.config_loader import load_config
from logger.custom_logger import CustomLogger
from exception.custom_exception import CustomException


def getCulinaryTeam(model_client):
    ...
    agent_map = {
        ...
        "RecipeAgent": getRecipeAgent,
        "CritiqueAgent" : getCritiqueAgent,
        "CulinaryTeamUserApproval": getCulinaryTeamUserApproval
    }
    
    try:
        log.info("Defining Culinary Team")
        ## loading config
        config = load_config()
        ## termination constants
        stop_word = config['termination']['word']
        max_message_turns = config['termination']['max_message_turns']
        
        ## team info
        team_info = config['teams']['CulinaryTeam']
        ...
        for agent_name in team_members:
            ...
            if agent_name =='CulinaryTeamUserApproval':
        ...
        termination_condition = TextMentionTermination(stop_word) | MaxMessageTermination(max_message_turns)
        
        culinary_team = RoundRobinGroupChat(
            participants=participants,
            termination_condition=termination_condition
        )
        log.info("Culinary Team Defined", members=team_members)
        
        return culinary_team
        
    except Exception as e:
        log.error("Error Initializing Culinary Team", team_info=team_info)
        raise CustomException("Error Initializing Culinary Team",sys)
...
if __name__ =='__main__':
    ...
    async def run_code():
        ...
        team = getCulinaryTeam(model_client)
